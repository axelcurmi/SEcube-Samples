...
uint16_t L0_TXRX(se3_device* device, uint16_t req_cmd, uint16_t req_cmdflags, uint16_t req_len, const uint8_t* req_data, uint16_t* resp_status, uint16_t* resp_len, uint8_t* resp_data) {
	...
	//uint16_t u16tmp = 0;
    //SE3_GET16(resp_data, SE3_RESP1_OFFSET_STATUS, u16tmp);
    //rintf("Valore di ritorno L0_TXRX prima di L0_RX-> %d\n", u16tmp);
    ...
    if (error != SE3_OK) {
        //SE3_GET16(resp_data, SE3_RESP1_OFFSET_STATUS, u16tmp);
        //printf("Valore di ritorno L0_TXRX nella risposta-> %d\n", u16tmp);
        return(error);
    }
...
static uint16_t L0_RX(se3_device* device, uint16_t* resp_status, uint16_t* resp_len, uint8_t* resp_data) {
    ...
    *resp_len = len;
    ...
    //printf("[device->reponse] Status %d - Len %d\n", *resp_status, *resp_len);

    if(resp_data!=NULL)
    {
        SE3_GET16(resp_data, SE3_RESP_OFFSET_STATUS, u16tmp);
        //printf("[resp_data] Status %d - ", u16tmp);
        SE3_GET16(resp_data, SE3_RESP_OFFSET_LEN, u16tmp);
        //printf("Len %d\n", len);
    }
