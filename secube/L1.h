...
/**
 *  \brief Initialize a protocol run
 *
 *  \param [in] s Pointer to current se3_session, you must be logged in
 *  \param [in] mode Which protocol to use
 *  \param [in] payload Pointer tho the payload, which will be sent
 *  \param [in] payload_size Size of the sent payload
 *  \return Error code or SE3_OK
 */
uint16_t L1_protocol_init(se3_session* s, uint16_t mode, uint8_t* payload, uint16_t payload_size);

/**
 *  \brief Update a protocol run
 *
 *  \param [in] s Pointer to current se3_session, you must be logged in
 *  \param [in] mode Which protocol to use
 *  \param [in] payload Pointer tho the payload, which will be sent
 *  \param [in] payload_size Size of the sent payload
 *  \param [out] response Pointer tho the response payload - if no repsonse is expected, set it to NULL
 *  \param [out] response_size Size of the response payload - if no repsonse is expected, set it to NULL
 *  \return Error code or SE3_OK
 */
