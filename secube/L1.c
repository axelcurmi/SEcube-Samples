...
//L1_TXRX(s, SE3_CMD1_KEY_LIST, 0, SE3_CMD1_KEY_LIST_REQ_SIZE, &resp_len);
...
uint16_t L1_login(se3_session* s, se3_device* dev, const uint8_t* pin, uint16_t access, bool logout) {
	...
	if (error != SE3_OK) {
        if(logout)	//if user want to force login -> check if the error is SE3_ERR_ACCESS or SE3_ERR_OPENED
        {
            if(error==SE3_ERR_ACCESS || error==SE3_ERR_OPENED)
            {
                printf("Try to force logout...\n");
                ...
                //login again
                error = L1_login(s, dev, pin, access, false);
                if ( error != SE3_OK ) {
                    return error;
                }
                else
...
uint16_t key_list(se3_session* s, uint16_t skip, uint16_t max_keys, se3_key* key_array, uint16_t* count) {
	...
	//memcpy(session_data + SE3_CMD1_KEY_LIST_REQ_OFF_NMAX, &max_keys, sizeof(max_keys));
	//memcpy(session_data + SE3_CMD1_KEY_LIST_REQ_OFF_SKIP, &skip, sizeof(skip));
	...
	for (i = 0; i < n_keys; i++) {   // Fills with key received
		...
		offset_key += SE3_CMD1_KEY_LIST_KEYINFO_OFF_NAME + key_array[i].name_size ;   // id + validity + data_size + name_size + name[name_size] + fingerprintsize
	}
...
uint16_t L1_protocol_update(se3_session* s, uint16_t mode, uint8_t* payload, uint16_t payload_size, uint8_t* response, uint16_t* response_size)
{
    uint16_t error = 0;
    uint16_t resp_len = 0;
    uint32_t u32tmp = 0;
    ...
    uint16_t req_len = 4 + payload_size;

    SE3_SET16(session_data, 0, mode);
    SE3_SET16(session_data, 2, payload_size);

    if (payload_size > 0)
    {
        memcpy(session_data + 4, payload, payload_size);
    }

    error = L1_TXRX(s, SE3_CMD1_PROTOCOL_UPDATE, 0, req_len, &resp_len);
    if (error != SE3_OK) {
        return error;
    }
    ...
    if (response != NULL)
    {
        memcpy(response, session_data, *response_size);
    }

    return error;
}

uint16_t L1_protocol_init(se3_session* s, uint16_t mode, uint8_t* payload, uint16_t payload_size)
{
    uint16_t error = 0;
    uint16_t resp_len = 0;
    uint32_t u32tmp = 0;
    ...
    //set mode (protocol type), payload size
    SE3_SET16(session_data, 0, mode);
    SE3_SET16(session_data, 2, payload_size);
    ...
    // Send data
    error = L1_TXRX(s, SE3_CMD1_PROTOCOL_INIT, 0, req_len, &resp_len);
